  ...
  if (rt.is_reg()) {
    ...
    if (FLAG_riscv_c_extension && is_int6(rt.immediate()) &&
      ...
      c_addi16sp(static_cast<int16_t>(rt.immediate()));
               ...
               (rs == sp) && is_uint10(rt.immediate()) && (rt.immediate()&0x3)==0 && 
...
void TurboAssembler::BranchShortHelper(int32_t offset, Label* L, bool apply_c_extension) {
  DCHECK(L == nullptr || offset == 0);
  offset = GetOffset(offset, L, OffsetSize::kOffset21);
  if(L != nullptr && !L->is_bound()) j(offset);
  else j(offset, apply_c_extension);
}
...
void TurboAssembler::BranchShort(Label* L, bool apply_c_extension) { BranchShortHelper(0, L, apply_c_extension); }
  ...
  }
  {
    ...
    switch (cond) {
      case cc_always:
        ...
        //c_ext = FLAG_riscv_apply_c_extension;
        if(L != nullptr && !L->is_bound()) j(offset);
        else j(offset, true);
...
void TurboAssembler::BranchAndLink(int32_t offset, bool apply_c_extension) {//the second arg is redundant
...
void TurboAssembler::BranchAndLink(Label* L, bool apply_c_extension) {//the second arg is redundant
  if (L->is_bound()) {
    if (is_near(L)) {
...
void TurboAssembler::Jump(Register target, bool apply_c_extension, Condition cond, Register rs,
                          const Operand& rt) {
  BlockTrampolinePoolScope block_trampoline_pool(this);
  if (cond == cc_always) {
...
void TurboAssembler::Jump(intptr_t target, RelocInfo::Mode rmode, bool apply_c_extension,
...
void TurboAssembler::Jump(Address target, RelocInfo::Mode rmode, bool apply_c_extension, Condition cond,
                          Register rs, const Operand& rt) {
...
void TurboAssembler::Jump(Handle<Code> code, RelocInfo::Mode rmode,
                           bool apply_c_extension, Condition cond, Register rs, const Operand& rt) {
  DCHECK(RelocInfo::IsCodeTarget(rmode));
  ...
  Jump(static_cast<intptr_t>(target_index), rmode, apply_c_extension, cond, rs, rt);
}

void TurboAssembler::Jump(const ExternalReference& reference, bool apply_c_extension) {
  li(t6, reference);
  Jump(t6, apply_c_extension);
}

// Note: To call gcc-compiled C code on riscv64, you must call through t6.
void TurboAssembler::Call(Register target, bool apply_c_extension, Condition cond, Register rs,
                          const Operand& rt) {
  BlockTrampolinePoolScope block_trampoline_pool(this);
  if (cond == cc_always) {
...
void TurboAssembler::Call(Address target, RelocInfo::Mode rmode, bool apply_c_extension, Condition cond,
                          Register rs, const Operand& rt) {
...
void TurboAssembler::Call(Handle<Code> code, RelocInfo::Mode rmode,
  ...
  int32_t target_index = AddCodeTarget(code);
  Call(static_cast<Address>(target_index), rmode, apply_c_extension, cond, rs, rt);
}
...
void MacroAssembler::JumpToExternalReference(const ExternalReference& builtin,
  ...
  Jump(code, RelocInfo::CODE_TARGET, false, al, zero_reg, Operand(zero_reg));
}
...
void TurboAssembler::CallForDeoptimization(Builtin target, int, Label* exit,
  ...
  if (kind == DeoptimizeKind::kEagerWithResume) {
    ...
    //   DCHECK_EQ(SizeOfCodeGeneratedSince(exit),
    //           Deoptimizer::kEagerWithResumeBeforeArgsSize - kShortInstrSize);
    // }
    // else{
    //   DCHECK_EQ(SizeOfCodeGeneratedSince(exit),
    //           Deoptimizer::kEagerWithResumeBeforeArgsSize);
